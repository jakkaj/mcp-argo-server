...
func registerResultTool(builder *app.Builder) *app.Builder {
	return builder.WithTool(func(wfClient wfclientset.Interface, logger *zap.Logger) fxctx.Tool {
		...
		handler := func(args map[string]interface{}) *mcp.CallToolResult {
			...
			if err != nil {
				logger.Error("Failed to get workflow for result", zap.Error(err))
				return errorResult(fmt.Sprintf("Failed to get workflow: %v", err))
			}

			phase := string(wf.Status.Phase)
			if phase != "Succeeded" {
				logger.Info("Requested results for incomplete or failed workflow",
			...
			logger.Info("Workflow outputs fetched", zap.String("name", name), zap.String("phase", phase))
